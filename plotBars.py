...
# compare band gaps with a bar chart
...
def getEdges(
    ...
    for root in root_list:
        ...
        edge_dict[natoms] = min(top_list), max(bot_list)
...
def getGaps(
    ...
    root: directory containing VASP output files for pristine system
    """
    bar_dict = getBars(ebounds, kpoint, printInfo, thres)
    edge_dict = getEdges(ebounds, thres, printInfo)
...
def plotBars(
    ...
    printInfo: if True, prints band gaps and Fermi energies as they are read (bool)
    """
    # UNDER CONSTRUCTION: show fermi level values
    bar_dict = getBars(ebounds, kpoint, printInfo, thres)
    edge_dict = getEdges(ebounds, thres, printInfo)
    ...
    for natoms, n in zip(natoms_list, range(numBars)):
        ...
        # plot bars
        top_list, bot_list, efermi = bar_dict[natoms]
        barColor = next(ax._get_lines.prop_cycler)['color'] # same color for each concentration
        ...
        # check if bands' edges are significatnly different from bars' edges
        ...
        if showFermi:
            ...
            ax.plot(endPointsX, endPointsY, color = 'black', linewidth = 2, linestyle = 'dashed')
